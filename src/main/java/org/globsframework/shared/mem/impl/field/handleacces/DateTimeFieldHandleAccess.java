...
public class DateTimeFieldHandleAccess implements HandleAccess {
    ...
    public static HandleAccess create(GroupLayout groupLayout, DateTimeField field) {
        return new DateTimeFieldHandleAccess(
                groupLayout.varHandle(MemoryLayout.PathElement.groupElement(field.getName() + DefaultOffHeapService.DATE_TIME_DATE_SUFFIX)),
                groupLayout.varHandle(MemoryLayout.PathElement.groupElement(field.getName() + DefaultOffHeapService.DATE_TIME_TIME_SUFFIX)),
                groupLayout.varHandle(MemoryLayout.PathElement.groupElement(field.getName() + DefaultOffHeapService.DATE_TIME_NANO_SUFFIX)),
                groupLayout.varHandle(
                        MemoryLayout.PathElement.groupElement(field.getName() + DefaultOffHeapService.DATE_TIME_ZONE_ID_SUFFIX),
    ...
    public void save(Glob data, MemorySegment memorySegment, long offset, SaveContext saveContext) {
        ...
        if (zone.length() > DefaultOffHeapService.DATE_TIME_MAX_ZONE_ID_SIZE) {
            throw new RuntimeException("Zone can not have more than " + DefaultOffHeapService.DATE_TIME_MAX_ZONE_ID_SIZE +" characters");
        }
        for (int i = 0; i < zone.length(); i++) {
            zoneIdVarHandle.set(memorySegment, offset, i, (byte)zone.charAt(i));
        }
        for (int i = zone.length(); i < DefaultOffHeapService.DATE_TIME_MAX_ZONE_ID_SIZE; i++) {
    ...
    public void readAtOffset(MutableGlob data, MemorySegment memorySegment, long offset, ReadContext readContext) {
        ...
        byte[] array = new byte[DefaultOffHeapService.DATE_TIME_MAX_ZONE_ID_SIZE];
        int len = 0;
        for (int i = 0; i < DefaultOffHeapService.DATE_TIME_MAX_ZONE_ID_SIZE; i++) {
