...
//    public static final int MODULO = 500;
    ...
    public void testStringWithFixSize() throws IOException {
        ...
        final Path path = Path.of("/tmp/test");
        Files.createDirectories(path);
        OffHeapWriteTreeService offHeapWriteTreeService =
                offHeapService.createWrite(path);

        offHeapWriteTreeService.save(globs);

        offHeapWriteTreeService.close();
        OffHeapReadTreeService readHeapService = offHeapService.createRead(
                Path.of("/tmp/test"), arena);
    ...
    private static void getMultiIndex(OffHeapReadTreeService readHeapService, OffHeapNotUniqueIndex offHeapMultiIndex, int loop, FunctionalKey[] functionalKeys) {
    ...
    private static void findByIndex(int loop, FunctionalKey[] functionalKeys, OffHeapReadTreeService readHeapService, OffHeapUniqueIndex offHeapUniqueIndex) {
        long startIndex = System.nanoTime();
        for (int i = 0; i < loop; i++) {
            for (FunctionalKey functionalKey : functionalKeys) {
